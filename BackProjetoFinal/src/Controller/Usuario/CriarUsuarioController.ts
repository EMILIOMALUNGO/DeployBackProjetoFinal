...
class CriarUsuarioController{
   async handle (req:Request, res:Response){
    ...
    const {nome, email , cpf_cnpj, rg_Ie, celular, fixo, rua, complemento ,cep,bairro,cidade, estado,password} = req.body
   // console.log(nome,email,password)
    ...
    if (!nome ||! email ||!cpf_cnpj || !rg_Ie || !celular|| !fixo || !rua ||!complemento || !cep || !bairro|| !cidade ||! estado ||!password) {
        ...
        const  CriarUsuario = await criarUsuarioacontroller.execute({
            nome, email , cpf_cnpj, rg_Ie, celular, fixo, rua, complemento ,cep,bairro,cidade, estado,password
        })
